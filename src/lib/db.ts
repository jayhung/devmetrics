...
const dbPath = path.join(process.cwd(), "data", "devmetrics.db");

let db: Database.Database | null = null;

/**
 * Migrates from old database filename if it exists.
 * Renames dashboard.db to devmetrics.db automatically on first run.
 */
function migrateDbIfNeeded() {
  const oldDbPath = path.join(process.cwd(), "data", "dashboard.db");
  ...
  if (fs.existsSync(oldDbPath) && !fs.existsSync(dbPath)) {
    ...
    if (fs.existsSync(oldWalPath)) {
      fs.renameSync(oldWalPath, dbPath + "-wal");
    }
    if (fs.existsSync(oldShmPath)) {
      fs.renameSync(oldShmPath, dbPath + "-shm");
    }
    console.log("Migrated database from dashboard.db to devmetrics.db");
  }
}
...
export function getRepoSyncState(repoId: number) {
  return getDb().prepare("SELECT * FROM repo_sync_state WHERE repo_id = ?").get(repoId) as
    | { repo_id: number; last_commit_sync: string | null; last_pr_sync: string | null }
    | undefined;
}
